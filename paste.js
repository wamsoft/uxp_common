...
//
// Windows では、パネルを開いた直後のプラグイン UI にキーボードが配送されず、
// Ctrl+V が届かない (Adobe 既知の不具合。macOS では起きない)。
// ボタンのクリックは UXP が処理するので OS のキーボード配送を必要としない。
// クリップボードはパネル側 (manifest で clipboard 権限を持つ) に読んでもらい、
// 直前に触った入力欄へ差し込む。
//
// 「直前に触った欄」で足りるのは、OS のキーボードフォーカスが来なくても
// クリックで DOM のフォーカスは動くため。実測でも、打鍵が届かない状態で
// focus イベントだけは入力欄まで来ていた。
//
// 打鍵そのものの手当ては wake.js (一度きりの案内) が持つ。
//---------------------------------------------------------------------------
